...
import {MAX_PRECISION_IN_AGENT_AMOUNTS} from './AccountingGlobals'
...
export async function calculateCommunityPotSplit (communityId: string, communityBalance: number):
  ...
  const membersCount = points.length
  const splitAmount = floorWithPrecision(Decimal(communityBalance).div(membersCount), MAX_PRECISION_IN_AGENT_AMOUNTS)

  const amounts = points.map(p => {
    const pn = Object.assign({}, p)
    pn.amount = splitAmount.toNumber()
    return pn
  })
